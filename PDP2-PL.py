...
def lecteur(fname):
    ...
    pick=np.zeros(n,dtype=float)
    global dmax
    dmax=0
    for i in range(n):
        cox[i]=ws.cell(4+i,2).value
        coy[i]=ws.cell(4+i,3).value
        pick[i]=ws.cell(4+i,4).value
        dem[i]=ws.cell(4+i,5).value
    for i in range(n):
        for j in range(n):
        ...
        for i in range(n):
            for j in range(n):
                if x[i,j].solution_value()>0.9 : 
                    print('De ',i,'  vers',j,' valeur ',x[i,j].solution_value(),' ',f[i,j].solution_value(),' ',g[i,j].solution_value())
